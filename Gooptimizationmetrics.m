function main
    ...
    params.battery_capacity = 20;  % Battery capacity in Ah
    ...
    for i = 1:length(locations)
        ...
        [~, ~, ~, metrics_history] = GO(popsize, dimension, xmax, xmin, MaxFEs, P_d, Cg_t, params);

        all_metrics_history.(locations{i}) = metrics_history;
    end

    % Plot metrics
    metrics_names = {'Battery_degradation', 'peak_to_average', 'reliability_cost', 'resiliency_cost', 'energy_cost' , 'Battery_automation'};
    plot_titles = {'Battery Degradation', 'Peak to Average Ratio', 'Reliability Cost', 'Resiliency Cost', 'Energy Cost' , 'Battery Automation'};

    for k = 1:2:length(metrics_names)
        figure('Units', 'normalized', 'Position', [0.1, 0.1, 0.8, 0.8]);
        subplot(2,1,1);
        hold on;
        for i = 1:length(locations)
            plot(all_metrics_history.(locations{i}).(metrics_names{k}), colors{i}, 'LineWidth', 2);
        end
        title(plot_titles{k}, 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        xlabel('Evaluations', 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        ylabel('Cost', 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        ...
        hold on;
        for i = 1:length(locations)
            plot(all_metrics_history.(locations{i}).(metrics_names{k+1}), colors{i}, 'LineWidth', 2);
        end
        title(plot_titles{k+1}, 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        xlabel('Evaluations', 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        ylabel('Cost', 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        legend(locations, 'FontName', 'Times New Roman', 'FontSize', 20, 'Location', 'best');
        set(gca, 'FontSize', 20, 'FontName', 'Times New Roman', 'FontWeight', 'bold');
    ...
    % Print final metrics
    disp('Final Metrics for each location:');
    for i = 1:length(locations)
        metrics_at_end = get_final_metrics(all_metrics_history.(locations{i}));
        fprintf('\n%s:\n', locations{i});
        fprintf('Energy Cost: %.2f\n', metrics_at_end.energy_cost);
        fprintf('Peak to Average: %.2f\n', metrics_at_end.peak_to_average);
        fprintf('Reliability Cost: %.2f\n', metrics_at_end.reliability_cost);
        fprintf('Resiliency Cost: %.2f\n', metrics_at_end.resiliency_cost);
        fprintf('Battery Degradation: %.2f\n', metrics_at_end.Battery_degradation);
        fprintf('Battery Automation: %.2f\n', metrics_at_end.Battery_automation);
        fprintf('Power Imbalance Penalty: %.2f\n', metrics_at_end.power_imbalance_penalty);
        fprintf('Battery Capacity Penalty: %.2f\n', metrics_at_end.battery_capacity_penalty);
    end
end
...
function metrics_at_end = get_final_metrics(metrics_history)
    metric_names = fieldnames(metrics_history);
    for i = 1:numel(metric_names)
        metrics_at_end.(metric_names{i}) = metrics_history.(metric_names{i})(end);
    end
end
