function main
    ...
    for i = 1:length(locations)
        ...
        % Store the final metrics from the last iteration
    ...
    hold on;
    for i = 1:length(locations)
        plot(timeAxis, all_P_batt(i, :), colors{i}, 'LineWidth', 2);
    end
    ...
    hold on;
    for i = 1:length(locations)
        plot(timeAxis, all_P_grid(i, :), colors{i}, 'LineWidth', 2);
    end
    ...
    dotted_colors = {'k', 'b', 'r', 'g', 'm'};
    
    % Plot Power Balance
    hold on;
    for i = 1:length(locations)
        % Normalize the load profile for plotting
        P_d = load_profiles{i};
        P_d_max = max(P_d);
        P_d = P_d / P_d_max;
    
        plot(timeAxis, P_d, [dotted_colors{i}, '--'], 'LineWidth', 3); % Thicker dotted lines
        plot(timeAxis, all_P_batt(i, :) + all_P_grid(i, :), colors{i}, 'LineWidth', 2);
    end
    ...
    legend([strcat(locations, ' Load'), strcat(locations, ' Total')], 'FontName', 'Times New Roman', 'FontSize', 20, 'Location', 'best');
    set(gca, 'FontSize', 20, 'FontName', 'Times New Roman', 'FontWeight', 'bold');
    datetick('x', 'HH:MM', 'keepticks');
    grid on;
    ...
    hold on;
    for i = 1:length(locations)
        plot(timeAxis, all_soc_history(i, :), colors{i}, 'LineWidth', 2);
    end
    ...
    for i = 1:length(locations)
        fprintf('\n%s:\n', locations{i});
        fprintf('Energy Cost: %f\n', all_metrics.energy_cost(i));
        fprintf('Peak to Average: %f\n', all_metrics.peak_to_average(i));
        fprintf('Reliability Cost: %f\n', all_metrics.reliability_cost(i));
        fprintf('Resiliency Cost: %f\n', all_metrics.resiliency_cost(i));
        fprintf('Battery Degradation: %f\n', all_metrics.Battery_degradation(i));
        fprintf('Battery Automation: %f\n', all_metrics.Battery_automation(i));
        fprintf('Power Imbalance Penalty: %f\n', all_metrics.power_imbalance_penalty(i));
        fprintf('Battery Capacity Penalty: %f\n', all_metrics.battery_capacity_penalty(i));
...
function [gbestX, gbestfitness, gbesthistory, metrics_history] = GO(popsize, dimension, xmax, xmin, MaxFEs, P_d, Cg_t, params)
    ...
    % Ensure that the final metrics are correctly captured
    if FEs == MaxFEs
        [~, final_metrics] = combined_objective(gbestX, P_d, Cg_t, params);
        metrics_history = update_metrics_history(metrics_history, final_metrics, FEs);
    end
end
