function run_PSO
    ...
    [best_solution, best_fitness, fitness_history, metrics_history] = runPSO(2 * T, xmin, xmax, P_d, Cg_t, params);
...
function [best_solution, best_fitness, fitness_history, metrics_history] = runPSO(dimension, xmin, xmax, P_d, Cg_t, params)
    ...
    options = optimoptions('particleswarm', ...
        'SwarmSize', 500, ...
        'MaxIterations', 10000, ...
        'OutputFcn', @(optimValues, flag) psoOutputFcn(optimValues, flag, P_d, Cg_t, params), ...
        'Display', 'iter');
    
    [best_solution, best_fitness] = particleswarm(@(x) combined_objective_wrapper(x, P_d, Cg_t, params), ...
        dimension, xmin, xmax, options);
...
function stop = psoOutputFcn(optimValues, flag, P_d, Cg_t, params)
    global fitness_history_global metrics_history_global;
    stop = false;
    switch flag
        case 'iter'
            [cost, metrics] = combined_objective_wrapper(optimValues.bestx, P_d, Cg_t, params);
...
function [cost, metrics] = combined_objective(x, P_d, Cg_t, params)
    ...
    cost = sum(objectives); % Combine the objectives into a single cost for PSO
