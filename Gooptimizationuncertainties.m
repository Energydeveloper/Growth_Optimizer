function main
    ...
    uncertainty_levels = [0.2];
    uncertainty_colors = {'r'};
    total_power_color = 'k';
    ...
    for i = 1:length(locations)
        ...
        figure('Name', locations{i}, 'Units', 'normalized', 'Position', [0.1, 0.1, 0.8, 0.8]);
        hold on;

        for u = 1:length(uncertainty_levels)
            % Add Gaussian uncertainty
            P_d_uncertainty = P_d .* (1 + uncertainty_levels(u) * randn(size(P_d)));
            Cg_t_uncertainty = Cg_t .* (1 + uncertainty_levels(u) * randn(size(Cg_t)));

            % Execute the Growth Optimizer
            [best_solution, best_fitness, ~, ~] = GO(popsize, dimension, xmax, xmin, MaxFEs, P_d_uncertainty, Cg_t_uncertainty, params);

            % Calculate the battery state of charge (SoC) in percentage
            P_batt = best_solution(1:2:end);
            P_grid = best_solution(2:2:end);

            P_total = P_batt + P_grid;
            ...
            % Plot bars for uncertain load demand
            bar(timeAxis, P_d_uncertainty, 'FaceColor', uncertainty_colors{u}, 'FaceAlpha', 0.5, 'EdgeColor', 'none');

            % Plot bars for total power on negative y-axis
            bar(timeAxis, -P_total, 'FaceColor', total_power_color, 'EdgeColor', 'none', 'LineStyle', '--');

            % Print the objective function value
            disp(['Objective function value for ', locations{i}, ' with ', num2str(uncertainty_levels(u)*100), '% uncertainty: ', num2str(best_fitness)]);
        end

        title(['Power Balance under Uncertainty - ', locations{i}], 'FontName', 'Times New Roman', 'FontSize', 24, 'FontWeight', 'bold');
        ...
        legend({'20% Uncertainty Load Demand + Tariff Price', 'Total Grid and Battery Power'}, 'FontName', 'Times New Roman', 'FontSize', 20, 'Location', 'best');
